 ...
 * @author Meijad
 */
public class BinaryHeapTester01 {

	@Test
	public void test01() { //Check the method size()
		BinaryHeap<String,Integer> H = new MinHeap<>();
		assertEquals( H.size(), 0 );
	}
	
	@Test
	public void test02() { //Check the methods Clear() and Empty()
		BinaryHeap<Integer, Integer> H = new MinHeap<>();
	...
	public void test08() { //check Iterator(), clone(), find(), and contains()
		...
		for ( int i = 0; i < 500; i ++ )
			H.add( i, ( int ) ( rand.nextInt( ( max - min ) + 1 ) + min ) );
		BinaryHeap<Integer, Integer> H2 = new MinHeap<>();
		try {
			H2 = ( MinHeap<Integer, Integer> ) H.clone();
