 ...
 * @param <T> A class of Points2D object.
 * @see <a href="https://en.wikipedia.org/wiki/K-d_tree">K-D Tree (Wikipedia)</a
 * @see java.lang.Iterable
 * @see java.lang.Comparable
 * @see data_structures.KDTNode
 * @see data_structures.Points2D
 */
public class KDTree<T extends Points2D> implements Iterable<T> , Comparable<KDTree<T>> {
	/**
	 ...
	 */
	public KDTree( KDTree<Points2D> newKDT ) {
		super();
	 ...
	 */
	public KDTree( Collection<Points2D> list ) {
		super();
		if ( list == null || list.size() == 0 )
			setRoot( null );
		else {
			KDTree<Points2D> T = readList( list );
			setRoot( T.getRoot() );
			setN( T.size() );
		}
	}
	 ...
	 * @param p Points2D object holding a coordinate x and y. 
	 ...
	 */
	public boolean remove( Points2D p ) {
		if ( isEmpty() )
			return false;
		...
		if ( removingNode.getLesser() == null && removingNode.getGreater() == null ) // Implementing level 1 in O(1).
			...
			if ( replacementNode != null ){
				...
				KDTree<Points2D> newTree = recreatingTree( removingNode ); // O(N)
	...
	/**
	 * @see #remove( Points2D point )
	 * @param x A double value for x coordinate.
	 * @param y A double value for Y coordinate.
	 */
	public boolean remove( double x, double y ) {
		return remove( new Points2D( x, y ) ); 
	}
	 ...
	 * @see #findNode(KDTNode newNode)
	 */
	public boolean contains( Points2D point ) {
	...
	/**
	 * @see #contains( Points2D point )
	 * @param x A double value for x coordinate.
	 * @param y A double value for y coordinate.
	 */
	public boolean contains( double x, double y ) {
		return contains(  new Points2D( x, y ) );
	}
	 ...
	 * @param point Points2D object that this method looks for its nearest node.
	 * @return <b> Points2D</b></br> A point which has the closest distance to the given point or null.
	 * @see #findNearest( KDTNode r, Points2D point )
	 */
	public Points2D findNearest( Points2D point ) {
	...
	/**
	 * @see #findNearest( Points2D point )
	 * @param x A double value for x coordinate.
	 * @param y A double value for y coordinate.
	 */
	public Points2D findNearest( double x, double y ) {
		return findNearest( new Points2D( x, y ) );
	}
	...
	protected KDTNode findNode( KDTNode newNode ) {
		...
		while ( currentNode != null ) {
			...
			if ( comp > 0 )
				 ...
				 * 
				 * Points2D.Y.Comparator and X.Comparator compares both points' X and Y.
				 * For more details check the Points2D.java:
				 * 		public static Comparator<Points2D> X_Comparator = new Comparator<Points2D>() {...
				 * 		public static Comparator<Points2D> Y_Comparator = new Comparator<Points2D>() {...
				 */
				int comp2 = ( currentNode.getAxis() == Axis.XAxis )
						? Points2D.Y_Comparator.compare( currentNode.getPoint() , newNode.getPoint() )
						: Points2D.X_Comparator.compare( currentNode.getPoint() , newNode.getPoint() );
				if ( comp2 == 0 )
	 ...
	 * @return <b> boolean </br> true</b> if it gets completed successfully.</br> <b>false</b> if it finds <b> Duplication</b>.
	 * @see data_structures.Points2D#X_Comparator
	 * @see data_structures.Points2D#Y_Comparator
	 */
		...
		if ( parentNode == null ){// Inserting into an empty tree
			...
			if ( comp < 0 )
				...
				int comp2 = ( parentNode.getAxis() == Axis.XAxis )  
						? Points2D.Y_Comparator.compare( parentNode.getPoint() , newNode.getPoint() )
						: Points2D.X_Comparator.compare( parentNode.getPoint() , newNode.getPoint() );
				if ( comp2 == 0 )
	 ...
	 */
	protected KDTree<Points2D> recreatingTree( KDTNode r ) {
		List<Points2D> tempList = inOrderTravers( r ); // Retrieving all points as a list, O(N).
		 ...
		 */
		KDTree<Points2D> newTree = new KDTree<>( );
	 ...
	 * @param point Points2D object that this method looks for its nearest node.
	 * @return <b> Points2D</b></br> A point which has the closest distance to the given point.
	 * @see #findNearest( Points2D point )
	 * @see data_structures.Points2D#GeographicalDistance(Points2D p1, Points2D p2)
	 * @see #pair()
	 */
	protected Points2D findNearest( KDTNode r, Points2D point ) {
	...
	protected void findNearest( KDTNode currentNode, KDTNode targetNode, Pair output ) {
		...
		if ( comp > 0 )
			...
			int comp2 = ( currentNode.getAxis() == Axis.XAxis )
					? Points2D.Y_Comparator.compare( currentNode.getPoint() , targetNode.getPoint() )
					: Points2D.X_Comparator.compare( currentNode.getPoint() , targetNode.getPoint() );
			if ( comp2 == 0 ) {
	 ...
	 * @see java.util.List
	 */
	protected KDTree<Points2D> readList( Collection<Points2D> list ) {
		KDTree<Points2D> Tree = new KDTree<>();
		readList( Tree, (List<Points2D>) list, 0, list.size() );
		return Tree;
	}
	 ...
	 */
	protected void readList( KDTree<Points2D> Tree, List<Points2D> list, int low, int high ) {
		if ( low < high ){
	...
	protected List<T> BFSTravers( KDTNode root ) {
		...
		List<Points2D> outputList = new ArrayList<>();
		Q.add( root );
		while( ! Q.isEmpty() ) {
			KDTNode u = Q.remove();
	 ...
	 * @param <T> A class of Points2D object.
	 * @return <b>List<T></b></br> A list from All coordinates.
	 * @see #add( Points2D point )
	 * @see java.util.LinkedList
	 * @see java.util.ArrayList
	 * @see java.util.List
	 * @see #remove(Points2D point)
	 */
	protected List<Points2D> inOrderTravers( KDTNode root ) {
		if ( root == null )
			 return null;
