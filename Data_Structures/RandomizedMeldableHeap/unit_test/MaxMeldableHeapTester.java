 ...
 * @author Meijad
 */
public class MaxMeldableHeapTester {
	@Test
	public void test01() { //Check the method size()
		MeldableHeap<String,Integer> H = new MaxMeldableHeap<>();
		assertEquals( H.size(), 0 );
	}
	
	@Test
	public void test02() { //Check the methods Clear() and Empty()
		MaxPriorityQueue<Integer, Integer> H = new MaxMeldableHeap<>();
		for ( int i = 0 ; i < 500 ; i ++ )
			H.add( i + 1, i );
		H.clear();
	...
	public void test03() { //Check the method Peek() and add(entity, priority)
		MeldableHeap<Integer, Integer> H = new MaxMeldableHeap<>();
	...
	public void test08() { //check Iterator(), clone(), find(), and contains()
		...
		for ( int i = 0; i < 500; i ++ )
			H.add( i, ( int ) ( rand.nextInt( ( max - min ) + 1 ) + min ) );
		MeldableHeap<Integer, Integer> H2 = new MaxMeldableHeap<>();
		try {
			H2 = ( MaxMeldableHeap<Integer, Integer> ) H.clone();
