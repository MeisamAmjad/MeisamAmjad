...
public class DictionaryTester {
	...
	public void constructor() {
		...
		assertTrue( "validate failed", D.validate() );
		assertTrue( "Dictionary size should be 0", D.size() == 0 );
	}
	
	/**
	 * Check that the first add doesn't crash,  modifies size correctly,
	 * and leaves the data structure intact.
	 */
	@Test
	public void firstadd() {
		Dictionary<Integer,Integer> D = create_dictionary( 0 );
		D.add( new Integer( 5 ), new Integer( 5 ) );
		assertTrue( "Wrong size after first add", D.size() == 1 );
		assertTrue( "validate failed", D.validate() );
	}
	
	/**
	 * Check that the only added element can be bound.
	 */
	@Test
	public void firstFind() {
		Dictionary<Integer,Integer> D = create_dictionary( 0 );
		D.add( new Integer( 5 ), new Integer( -1 ) );
		assertEquals( "Element not found", D.find( new Integer( 5 ) ), new Integer( -1 ) );
	}
	
	/**
	 * Check that multiple adds don't crash, modifies size correctly,
	 * and leaves the data structure intact.
	 */
	@Test
	...
	public void testFindPositive() {
		...
		for ( int i = 0; i < 10; i ++ ) {
			...
			assertTrue( "validate failed after D.find(" + 2 * i + ")", D.validate() );
		}
	}
	...
	/**
	 * Make sure we can add an element at the beginning.
	 */
	@Test
	public void testaddBegin() {
		Dictionary<Integer,Integer> D = create_dictionary( 10 );
		D.add( new Integer( 1 ), new Integer( -4 ) );
	...
	/**
	 * Make sure we can add an element in the middle.
	 */
	@Test
	...
	/**
	 * Make sure we can add an element at the end.
	 */
	@Test
	public void testaddEnd() {
		Dictionary<Integer,Integer> D = create_dictionary( 10 );
		D.add( new Integer( 50 ), new Integer( 51 ) );
		assertTrue( "Size should be 11", D.size() == 11 );
	...
	public void testClear() {
		...
		assertTrue( "validate failed after clear", D.validate() );
	}
	
	
	/**
	 * Test that the same key cannot be added twice.
	 */
	@Test
	public void testRepeatedKey() {
		Dictionary<Integer,Integer> D = create_dictionary( 10 );
		assertFalse( D.add( new Integer( 6 ), new Integer( 6 ) ) );
	}
	
	/**
	 * Test that the same value value be added twice.
	 */
	@Test
	public void testRepeatedValue() {
		Dictionary<Integer,Integer> D = create_dictionary( 10 );
		assertTrue( D.add( new Integer( 7 ), new Integer( 7 ) ) );
		assertFalse( D.add( new Integer( 8 ), new Integer( 8 ) ) );
	}
}
