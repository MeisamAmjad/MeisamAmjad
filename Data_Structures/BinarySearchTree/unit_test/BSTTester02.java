...
import data_structures.PairNode;
import data_structures.BST.breathFirstSearch;
import data_structures.BST.depthFirstSearch;
...
public class BSTTester02 {
	...
	private static void create_helper( BST<Integer,Integer> T, int low, int high ) {
		if ( low < high ) {
			int m = ( low + high ) / 2;
			T.add( new Integer( 2 * m ), new Integer( 2 * m + 1 ) );
	...
	public void _testHelper( int N, int I, int[] rmvItems, String whichMethod ){
		...
		ArrayList<PairNode<Integer, Integer>> A = new ArrayList<>();
		if ( "BFS".equals( whichMethod ) )	
			A = ( ArrayList<PairNode<Integer, Integer>> ) T.toCollection( breathFirstSearch.bFS );
		else if ( "preOrderTravers".equals( whichMethod ) )
			A = ( ArrayList<PairNode<Integer, Integer>> ) T.toCollection( depthFirstSearch.preOrder );
		else if ( "inOrderSortTravers".equals( whichMethod ) )
			A = ( ArrayList<PairNode<Integer, Integer>> ) T.toCollection( depthFirstSearch.inOrder );
		else if ( "postOrderRecursive".equals( whichMethod ) )
			A = ( ArrayList<PairNode<Integer, Integer>> ) T.toCollection( depthFirstSearch.postOrder );
		assertTrue( "The size of the Tree should be:" + N, T.size() == N );
		assertTrue( "The size of the ArrayList should be:" + N, A.size() == N );
		
		// Tests the output ArrayList after adding
	...
	public void constructor() {
		...
		assertTrue( "check_structure failed", T.validate() );
	...
	public void rootTest(){
		...
		assertNull( "It should return null", T.toCollection( breathFirstSearch.bFS ) );
		assertNull( "It should return null", T.toCollection( depthFirstSearch.preOrder ) );
		assertNull( "It should return null", T.toCollection( depthFirstSearch.inOrder ) );
		assertNull( "It should return null", T.toCollection( depthFirstSearch.postOrder ) );
	}
}
