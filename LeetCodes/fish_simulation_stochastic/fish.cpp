...
void MTRand_int32::readState(char *restartFileName) {
 int i;
 ifstream restartFile;

 restartFile.open(restartFileName,ios::in);
...
void MTRand_int32::dumpState(char *restartFileName) {
 int i;
 ofstream restartFile;

 restartFile.open(restartFileName,ios::trunc|ios::out);
...
int main (int argc, char* argv[]) {
    unsigned long seed;
    size_t trial = 50;
    int numYears,year,day,newNumberOfFish,initNumberOfFish;
    ...
    const std::string& filePath = "noFishing.txt";
    // Annual fishing rate
    annualFishingRate = 0.0;
    // Random number seed
    seed = strtoul("0", NULL, 10);
    irand.seed(seed);

    initNumberOfFish = numberOfFish;
    dailyAgeIncrement = 1.0/((double) DAYSPERYEAR);

    numberOfFish = initNumberOfFish;
